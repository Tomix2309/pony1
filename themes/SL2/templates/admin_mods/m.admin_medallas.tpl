		...
		{foreach from=$tsMedals.medallas item=m}
			<tr id="medal_id_{$m.medal_id}">
				...
				<td class="admin_actions">
					<a onclick="admin.medallas.asignar({$m.medal_id}); return false" title="Asignar Medalla"><i data-feather="at-sign"></i></a>
					<a href="{$tsConfig.url}/admin/medals/editar/{$m.medal_id}" title="Editar Medalla"><i data-feather="edit-3"></i></a>
					<a onclick="admin.medallas.borrar({$m.medal_id}); return false" title="Borrar Medalla"><i data-feather="trash"></i></a>
				</td>
			</tr>
		{/foreach}
		</tbody>
	...
	<table class="table table-striped">
		...
		{foreach from=$tsAsignaciones.asignaciones item=m}
			<tr id="assign_id_{$m.id}">
				...
				<td>{$m.medal_ip}</td>
				<td class="admin_actions">
					<a onclick="admin.medallas.borrar_asignacion({$m.id}, {$m.medal_id}); return false" title="Borrar Asignaci&oacute;n"><i data-feather="trash"></i></a>
				</td>
			</tr>
		{/foreach}
		</tbody>
