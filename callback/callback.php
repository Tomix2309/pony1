...
class Callback extends tsCore {
	...
	*/
	...
	public function getFacebookProfilePicture($userId) {
    	// Cambia 'large' según tus necesidades (puede ser 'small', 'normal', 'large', etc.)
    	$pictureUrl = "https://graph.facebook.com/v18.0/{$userId}/picture?type=large";
    	return $pictureUrl;
	}
	...
	public function cURLUser($data) {
		...
		// Agrega la URL de la imagen de perfil al objeto userData
    	if ($this->social === 'facebook') {
        	$userData->profile_picture = self::getFacebookProfilePicture($userData->id);
    	}
		return $userData;
	}
