...
class tsMonitor {
	...
	*/
    ...
    private function makeMonitor(){
        $this->monitor = array(
            ...
            18 => array('text' => array('coment&oacute; tu','_REP_ nuevos comentarios en tu'), 'ln_text' => 'archivo', 'css' => 'comment_post'),
            19 => array('text' => 'te mencion&oacute; en el', 'ln_text' => 'archivo', 'css' => 'blue_ball'),
            20 => array('text' => 'agreg&oacute; a favoritos tu', 'ln_text' => 'archivo', 'css' => 'star'),
        );
    }
    ...
    */
    private function makeOracion($data){
        ...
        $oracion['avatar'] = $data['obj_user'].'.webp';
        ...
        switch($no_type){
            ...
            break;
			   case 15:
               $oracion['text'] = 'Recibiste una nueva <span title="'.$data['m_title'].'"><b>medalla</b> <img src="'.$tsCore->settings['url'].'/themes/'.TS_TEMA.'/images/icons/med/'.$data['m_image'].'_32.png" style="width:15px;height:15px;" alt="'.$data['m_title'].'"/></span>';
			   break;
			   case 16:
               $oracion['text'] = 'Tu <a href="'.$site_url.'/posts/'.$data['c_seo'].'/'.$data['post_id'].'/'.$tsCore->setSEO($data['post_title']).'.html" title="'.$data['post_title'].'"><b>post</b></a> tiene una nueva <span title="'.$data['m_title'].'"><b>medalla</b> <img src="'.$tsCore->settings['url'].'/themes/'.TS_TEMA.'/images/icons/med/'.$data['m_image'].'_32.png"style="width:15px;height:15px;" alt="'.$data['m_title'].'"/></span>';
			   break;
			   case 17:
               $oracion['text'] = 'Tu <a href="'.$site_url.'/fotos/'.$data['user_name'].'/'.$data['foto_id'].'/'.$tsCore->setSEO($data['f_title']).'.html" title="'.$data['f_title'].'"><b>foto</b></a> tiene una nueva <span title="'.$data['m_title'].'"><b>medalla</b> <img src="'.$tsCore->settings['url'].'/themes/'.TS_TEMA.'/images/icons/med/'.$data['m_image'].'_32.png"style="width:15px;height:15px;" alt="'.$data['m_title'].'"/></span>';
			   break;
            case 18:
            case 20:
               // CUANTOS
               ...
               //
               $oracion['link'] = $site_url.'/files/'.$data['file_id'].'/'.$data['f_nombre'];
               $oracion['ltext'] = ($this->show_type == 1) ? $ln_text : $data['f_nombre'];
               $oracion['ltit'] = ($this->show_type == 1) ? $data['f_nombre'] : '';
            break;
            case 19:
               $oracion['text'] = $this->monitor[$no_type]['text'].$txt_extra;
               $oracion['link'] = $site_url.'/files/'.$data['file_id'].'/'.$data['f_nombre'];
	...
	*/
	public function getFollows($type, $user_id = 0){
		...
		switch($type){
			...
			break;
            case 'posts':
				$query = 'SELECT f.f_id, p.post_user, p.post_title, u.user_name, c.c_seo, c.c_nombre FROM u_follows AS f LEFT JOIN p_posts AS p ON f.f_id = p.post_id LEFT JOIN u_miembros AS u ON u.user_id = p.post_user LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE f.f_user = \''.(int)$user_id.'\' AND f.f_type = \'2\'ORDER BY f.f_date DESC';
                // PAGINAR
                $total = db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', $query));
                $pages = $tsCore->getPagination($total, 12);
                $data['pages'] = $pages;
