...
class tsMensajes {
   ...
   function __construct(){
      ...
      // MENSAJES
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(mp_id) AS total FROM u_mensajes WHERE mp_to = \''.$tsUser->uid.'\'  && mp_read_mon_to < \'2\' && mp_del_to = \'0\'');
      $data = db_exec('fetch_assoc', $query);
      
        $this->mensajes = $data['total'];
        // RESPUESTAS
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(mp_id) AS total FROM u_mensajes WHERE mp_answer = \'1\' && mp_from = \''.$tsUser->uid.'\' && mp_read_mon_from < \'2\' && mp_del_from = \'0\'');
      $data = db_exec('fetch_assoc', $query);
   ...
   function getValid(){
        ...
        //
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT user_id FROM u_miembros WHERE LOWER(user_name) = \''.$tsCore->setSecure($para).'\' LIMIT 1');        
   ...
   public function newMensaje() {
      ...
      if ($tsUser->is_member && $tsUser->info['user_baneado'] == 0 && $tsUser->info['user_activo'] == 1) {
         ...
         if (!empty($user_id) && !empty($mensaje)) {
            //BLOQUEADO
            if (!$tsUser->is_admod) {
               $block = db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT bid FROM u_bloqueos WHERE (b_user = \'' . $user_id . '\' && b_auser = \'' . $tsUser->uid . '\') || (b_user = \'' . $tsUser->uid . '\' && b_auser = \'' . $user_id . '\') LIMIT 1'));
               if ($block) die('No puedes enviarle mensajes a ' . $para);
            }
            //VISTA PREVIA
            $preview = substr($mensaje, 0, 75);
            $com = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', 'SELECT user_activo, user_baneado FROM u_miembros WHERE LOWER(user_name) = \'' . $tsCore->setSecure($para) . '\''));
            if ($com['user_activo'] != 0 && $com['user_baneado'] != 1) {
               $comp = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', 'SELECT (SELECT COUNT(follow_id) FROM u_follows WHERE f_id = \'' . (int) $user_id . '\' AND f_user = \'' . (int) $tsUser->uid . '\' AND f_type = \'1\' LIMIT 1) as lesigo, (SELECT COUNT(follow_id) FROM u_follows WHERE f_id = \'' . (int) $tsUser->uid . '\' AND f_user = \'' . (int) $user_id . '\' AND f_type = \'1\' LIMIT 1) as mesigue, (SELECT COUNT(user_id) FROM u_miembros WHERE user_id = \'' . $user_id . '\' AND user_rango < \'2\' LIMIT 1) as noesunadmin'));
               if (!$comp['noesunadmin']) {
               ...
               if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `u_mensajes` (`mp_to`, `mp_from`, `mp_subject`, `mp_preview`, `mp_date`) VALUES (' . $user_id . ', ' . $tsUser->uid . ', \'' . $asunto . '\', \'' . $tsCore->setSecure($preview, true) . '\',\'' . time() . '\')')) {
                  ...
                  if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `u_respuestas` (`mp_id`, `mr_from`, `mr_body`, `mr_ip`, `mr_date`) VALUES (\'' . (int) $mp_id . '\', \'' . $tsUser->uid . '\', \'' . $tsCore->setSecure($mensaje, true) . '\',  \'' . $_SERVER['REMOTE_ADDR'] . '\', \'' . time() . '\')')) {
                     return 'El mensaje ha sido enviado a <a class="font-weight-bold text-white" href="' . $tsCore->settings['url'] . '/perfil/' . $para . '">' . $para . '</a>. <center><a class="btn btn-block btn-info resp" href="' . $tsCore->settings['url'] . '/mensajes/leer/' . $mp_id . '">Ver el mensaje enviado</a></center>';
                  } else {
   ...
   public function newRespuesta()
   {
      ...
      //
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT `mp_to`, `mp_from`, `mp_answer` FROM `u_mensajes` WHERE `mp_id` = \'' . (int) $mp_id . '\' ' . ($tsUser->is_admod ? '' : '&& mp_del_to = \'0\' && mp_del_from = \'0\'') . ' LIMIT 1');
      ...
      if (!empty($msg)) {
         ...
         // BLOQUEADO
         if (!$tsUser->is_admod) {
            $block = db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT bid FROM u_bloqueos WHERE (b_user = \'' . $msg['mp_to'] . '\' && b_auser = \'' . $msg['mp_from'] . '\') || (b_user = \'' . $msg['mp_from'] . '\' && b_auser = \'' . $msg['mp_to'] . '\') LIMIT 1'));
         ...
         if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `u_respuestas` (`mp_id`, `mr_from`, `mr_body`, `mr_ip`, `mr_date`) VALUES (\'' . intval($mp_id) . '\', \'' . $tsUser->uid . '\', \'' . $tsCore->setSecure($mp_body, true) . '\', \'' . $_SERVER['REMOTE_ADDR'] . '\', \'' . time() . '\')')) {
            ...
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE `u_mensajes` SET `mp_preview` = \'' . $tsCore->setSecure($preview, true) . '\', `mp_date` = \'' . time() . '\' ' . $update . ' WHERE `mp_id` = \'' . (int) $mp_id . '\'');
            //
            $return['mp_date'] = time();
   ...
   public function getMensajes($type = 1, $unread = false, $where = 'normal')
   {
      ...
      if ($type == 1) {
         ...
         while ($row = db_exec('fetch_assoc', $query)) {
            ...
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE `u_mensajes` SET ' . $update . ' WHERE mp_id  = \'' . (int) $row['mp_id'] . '\'');
            //
            $data['total']++;
         }
   ...
   public function readMensaje()
   {
      ...
      //
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT m.*, u.user_name FROM `u_mensajes` AS m LEFT JOIN `u_miembros` AS u ON m.mp_from = u.user_id WHERE m.mp_id = \'' . $mp_id . '\' ' . ($tsUser->is_admod ? '' : '&& ((m.mp_to = \'' . $tsUser->uid . '\' && m.mp_del_to = 0) || (m.mp_from = \'' . $tsUser->uid . '\' && m.mp_del_from = 0))') . ' LIMIT 1');
      ...
      if (db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT obj_id FROM `w_denuncias` WHERE obj_id = \'' . (int) $mp_id . '\' && `d_type` = \'2\' LIMIT 1')) && $tsUser->is_admod) {$canview = true;} else { $canview = false;}
      ...
      if ($tsUser->uid == $data['mp_to']) {
         $update                    = 'mp_read_to = 1, mp_read_mon_to = 2';
         $history['msg']['mp_type'] = 1;} // PARA MI
      elseif ($from == $data['mp_to'] && $data['mp_from'] == $tsUser->uid) {
         $update                    = 'mp_read_from = 1, mp_read_mon_from = 2';
         $history['msg']['mp_type'] = 2;} // ME RESPONDIERON
      ...
      // LEIDO
      if (isset($update)) {
         db_exec([__FILE__, __LINE__], 'query', 'UPDATE `u_mensajes` SET ' . $update . ' WHERE `mp_id` = \'' . intval($mp_id) . '\'');
      }
      // Fix: 27/02/2014

      // BLOQUEADO
      $user_id = ($data['mp_from'] != $tsUser->uid) ? $data['mp_from'] : $data['mp_to'];
      $query   = db_exec([__FILE__, __LINE__], 'query', 'SELECT bid AS block FROM `u_bloqueos` WHERE b_user = ' . $tsUser->uid . ' AND b_auser = \'' . intval($user_id) . '\' LIMIT 1');
      // PUEDO RESPONDER o estoy bloqueado?
      $history['ext']['can_read'] = (!$tsUser->is_admod && db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT bid FROM u_bloqueos WHERE (b_user = \'' . $data['mp_to'] . '\' && b_auser = \'' . $data['mp_from'] . '\') || (b_user = \'' . $data['mp_from'] . '\' && b_auser = \'' . $data['mp_to'] . '\') LIMIT 1')) > 0) ? 0 : 1;
   ...
   public function editMensajes()
   {
      ...
      switch ($act) {
         case 'read':
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_mensajes SET mp_read_to = 1, mp_read_mon_to = 2 WHERE mp_id IN(' . implode(',', $nids[1]) . ') AND mp_to = \'' . $tsUser->uid . '\' LIMIT 1');
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_mensajes SET mp_read_from = 1, mp_read_mon_from = 2 WHERE mp_id IN(' . implode(',', $nids[2]) . ') AND mp_from = \'' . $tsUser->uid . '\' LIMIT 1');
            break;
         case 'unread':
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_mensajes SET mp_read_to = 0, mp_read_mon_to = 1 WHERE mp_id IN(' . implode(",", $nids[1]) . ') AND mp_to = \'' . $tsUser->uid . '\' LIMIT 1');
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_mensajes SET mp_read_from = 0, mp_read_mon_from = 1 WHERE mp_id IN(' . implode(",", $nids[2]) . ') AND mp_from = \'' . $tsUser->uid . '\' LIMIT 1');
            break;
         case 'delete':
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_mensajes SET mp_del_to = 1 WHERE mp_id IN(' . implode(",", $nids[1]) . ') AND mp_to = \'' . $tsUser->uid . '\' LIMIT 1');
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_mensajes SET mp_del_from = 1 WHERE mp_id IN(' . implode(",", $nids[2]) . ') AND mp_from = \'' . $tsUser->uid . '\' LIMIT 1');
            ...
            while ($row = db_exec('fetch_assoc', $query)) {
               if (db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM `u_mensajes` WHERE `mp_id` =  \'' . $row['mp_id'] . '\'')) {
                  db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM `u_respuestas` WHERE `mp_id` = \'' . $row['mp_id'] . '\'');
               }
            }
            //
            break;
