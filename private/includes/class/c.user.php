...
class tsUser {
	...
	public $nick = 'Anonimo';// NOMBRE A MOSTRAR
	...
	*/
	...
	private function getTimeZone(int $uid = 0) {
		...
		if(!empty($zona["user_pais"]) || !empty($zona["user_estado"]))   {
			...
			include ROUTEEXTRAS . "geodata.php";
	...
	*/
	public function loadUser($login = FALSE) {
		$tsCore = new tsCore;
		// Cargar datos
		$this->info = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT u.*, s.* FROM u_sessions s, u_miembros u WHERE s.session_id = '{$this->session->ID}' AND u.user_id = s.session_user_id"));
		// Existe el usuario?
		...
		if($this->permisos['sumo'] == false && $this->permisos['suad'] == true) $this->is_admod = 1;
		elseif($this->permisos['sumo'] == true && $this->permisos['suad'] == false) $this->is_admod = 2;
		elseif($this->permisos['sumo'] || $this->permisos['suad']) $this->is_admod = true;
		else $this->is_admod = 0;
		
		// NOMBRE
		$this->nick = $this->info['user_name'];
		$this->uid = $this->info['user_id'];
		$this->avatar = $tsCore->getAvatar($this->uid);
	 ...
	 * @return array
	*/
	public function ConfigPortada() {
		$tsCore = new tsCore;
		return $tsCore->verify_background('perfil');
	}
	/**
	...
	*/
	public function loginUser(string $username = '', string $password = '', bool $remember = TRUE, $redirectTo = NULL) {
		$tsCore = new tsCore;

		/* ARMAR VARIABLES */
		$email = false;
		if (filter_var($username, FILTER_VALIDATE_EMAIL)) $email = true;
		$select = "user_" . ($email ? 'email' : 'name') . " = '$username'";
		/* CONSULTA */
		$data = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT user_id, user_name, user_password, user_activo, user_baneado FROM u_miembros WHERE $select LIMIT 1"));
		//
		...
		if($data['user_activo'] == 1) {
			...
			/* REDERIGIR */
			if(!empty($redirectTo)) $tsCore->redirectTo($redirectTo);
			return '1: Iniciando sesion...';
	...
	*/
	function userActivate($tsUserID = 0, $tsKey = 0){
		...
		//
		if(db_exec('num_rows', $query) == 0 || $tsKey != $tsKeyLocal) return false;
		else {
			if(db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_miembros SET user_activo = 1 WHERE user_id = \''.$tsUserID.'\'')) return $tsData;
			else return false;
		}
	}
	 /*
	 ...
	 */
	 function getUserBanned(){
		  //
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT * FROM u_suspension WHERE user_id = \''.$this->uid.'\' LIMIT 1');
	...
	*/
	...
	global $tsCore;
		...
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT user_id FROM u_miembros WHERE LOWER(user_name) = \''.$tsCore->setSecure($tsUsername).'\' LIMIT 1');
	 ...
	 */
	 function getUserName($user_id){
		//
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT user_name FROM u_miembros WHERE user_id = \''.(int)$user_id.'\' LIMIT 1');
		$tsUser = db_exec('fetch_assoc', $query);
	...
	*/
	public function getVCard($user_id){
		...
		// INFORMACION GENERAL
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_id, u.user_name, u.user_lastactive, u.user_baneado, p.user_sexo, p.user_pais, p.p_nombre, p.p_mensaje, p.p_sitio FROM u_miembros AS u, u_perfil AS p WHERE u.user_id = \''.(int)$user_id.'\' AND p.user_id = \''.(int)$user_id.'\'');
		$data = db_exec('fetch_assoc', $query);
		  
		//  STATS
		$query =  db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_puntos, r.r_name, r.r_color, r.r_image FROM u_miembros AS u LEFT JOIN u_rangos AS r ON u.user_rango = r.rango_id WHERE user_id = \''.(int)$user_id.'\' LIMIT 1');
		...
		$q1 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(post_id) AS p FROM p_posts WHERE post_user = \''.(int)$user_id.'\' && post_status = \'0\''));;
		  $q2 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(cid) AS c FROM p_comentarios WHERE c_user = \''.(int)$user_id.'\' && c_status = \'0\''));;
		$q3 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(follow_id) AS s FROM u_follows WHERE f_id = \''.(int)$user_id.'\' && f_type = \'1\''));
