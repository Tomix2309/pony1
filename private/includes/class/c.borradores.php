...
class tsDrafts {
	...
	*/
	function getDrafts(){
      ...
      $drafts['total_cat'] = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(b.b_category) AS total FROM p_borradores AS b '));
	...
	*/
	function getDraft($status = 1){
        ...
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT bid, b_user, b_date, b_title, b_portada, b_body, b_tags, b_category, b_private, b_block_comments, b_sponsored, b_sticky, b_smileys, b_post_id, b_status, b_causa FROM `p_borradores` WHERE `bid` = \''.(int)$bid.'\' AND `b_user` = \''.$tsUser->info['user_id'].'\' AND b_status = \''.$status.'\' LIMIT 1');
		//
