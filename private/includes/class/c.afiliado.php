...
class tsAfiliado {
    ...
    */
    function newAfiliado(){
        ...
        //
		if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `w_afiliados` (a_titulo, a_url, a_banner, a_descripcion, a_date) VALUES (\''.$tsCore->setSecure($dataIn['titulo']).'\', \''.$tsCore->setSecure($dataIn['url']).'\', \''.$tsCore->setSecure($dataIn['banner']).'\', \''.$tsCore->setSecure($dataIn['desc']).'\', \''.time().'\')')) {
		$afid = db_exec('insert_id');
		  // AVISO
            ...
            $return .= '<label class="form-label" for="atitle">C&oacute;digo HTML</label>';
            $return .= '<textarea tabindex="4" rows="10" style="height:60px;" class="form-input" onclick"select(this)">';
    ...
    */
    function urlOut(){
        global $tsCore;
        //
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT a_url FROM w_afiliados WHERE aid = \''.intval($_GET['ref']).'\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);
