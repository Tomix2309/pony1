...
class tsFotos {
	...
	*/
	function newFoto(){
        ...
        $q1 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(foto_id) AS f FROM `f_fotos` WHERE (f_date > \''.(time()-$antiflood).'\' && f_user = \''.$tsUser->uid.'\') OR (f_url = \''.$fData['foto']['url'].'\' OR (f_title = \''.$fData['titulo'].'\' && f_date > \''.(time()-$antiflood*12).'\' && f_user = \''.$tsUser->uid.'\') ) LIMIT 1'));
        ...
        if($result[0][0] == 0) return $result[0][1];
			...
			// INSERTAMOS
			db_exec([__FILE__, __LINE__], 'query', 'UPDATE `f_fotos` SET f_last = \'0\' WHERE f_user = '.$tsUser->uid.' AND f_last = 1'); // LA ULTIMA DEJA DE SERLO
            $_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
            if(!filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)) { die('0: Su ip no se pudo validar.'); }
			if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `f_fotos` (f_title, f_date, f_description, f_url, f_user, f_closed, f_visitas, f_last, f_ip) VALUES (\''.$fData['titulo'].'\', \''.time().'\', \''.$fData['desc'].'\',  \''.$img_url.'\', \''.$tsUser->uid.'\', \''.$fData['closed'].'\', \''.$fData['visitas'].'\', \'1\', \''.$_SERVER['REMOTE_ADDR'].'\')')) {
                $fid = db_exec('insert_id');
                // Estadísticas
                db_exec([__FILE__, __LINE__], 'query', 'UPDATE `w_stats` SET `stats_fotos` = stats_fotos + \'1\' WHERE `stats_no` = \'1\'');
                //db_exec([__FILE__, __LINE__], 'query', 'UPDATE `u_miembros` SET `user_fotos` = user_fotos + \'1\' WHERE `user_id` = \''.$tsUser->uid.'\''); // Eliminado en 1.1.000.9
    ...
    */
    function getFotoEdit(){
        ...
        // DATOS
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT * FROM `f_fotos` WHERE foto_id = \''.(int)$fid.'\' LIMIT 1');
    ...
    */
    function editFoto(){
        ...
        $fid = (int)$_GET['id'];
        // DATOS
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.foto_id, f.f_title, f.f_user, u.user_name FROM f_fotos AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.foto_id = \''.(int)$fid.'\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);
        ...
        if(!empty($data['f_user'])){
            ...
            if($data['f_user'] == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos['moedfo']){
                ...
                // UPDATES
				db_exec([__FILE__, __LINE__], 'query', 'UPDATE `f_fotos` SET f_title = \''.$fData['titulo'].'\', f_description = \''.$fData['desc'].'\',  f_closed = \''.$fData['closed'].'\', f_visitas = \''.$fData['visitas'].'\' WHERE foto_id = \''.(int)$fid.'\'');
				...
				if($data['f_user'] != $tsUser->uid){
				    ...
				    db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `w_historial` (`pofid`, `action`, `type`, `mod`, `reason`, `date`, `mod_ip`) VALUES (\''.(int)$data['foto_id'].'\', \'1\', \'2\', \''.$tsUser->uid.'\', \''.$fData['razon'].'\', \''.time().'\', \''.$tsCore->setSecure($_SERVER['REMOTE_ADDR']).'\')');
				}
				// REDIRIGIMOS
    ...
    */
    function delFoto(){
        ...
        // DATOS
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT `f_user` FROM `f_fotos` WHERE foto_id = \''.(int)$fid.'\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);
        ...
        if(!empty($data['f_user'])){
            ...
            if($data['f_user'] == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos['moef']){
			    if(db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM f_fotos WHERE foto_id = \''.(int)$fid.'\'')){
                    // BORRAMOS LOS COMENTARIOS
					db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM `f_comentarios` WHERE c_foto_id = \''.(int)$fid.'\'');
    ...
    */
    function getLastFotos(){
        ...
        $data['photos__total'] = $total; 
		
		//
		$query = 'SELECT f.foto_id, f_user, f.f_title, f.f_date, f.f_description, f.f_url, f.f_status, u.user_name, u.user_activo, u.user_baneado FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'WHERE f.f_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY f.foto_id DESC LIMIT '.$limit;
        $data['data'] = result_array(db_exec([__FILE__, __LINE__], 'query', $query));
		foreach ($data['data'] as $key => $foto) {
            $data['data'][$key]['f_avatar'] = $tsCore->getAvatar($foto['f_user']);
        }
        //
        return $data;
    }
    ...
    */
    function getLastComments(){
        ...
        //
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT c.cid, c.c_user, f.foto_id, f.f_title, f.f_status, u.user_id, u.user_name, u.user_activo FROM f_comentarios AS c LEFT JOIN f_fotos AS f ON c.c_foto_id = f.foto_id LEFT JOIN u_miembros AS u ON f.f_user = u.user_id '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'WHERE f.f_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY c.c_date DESC LIMIT 10');
    ...
    */
    function getFoto(){
        ...
        $fid = intval($_GET['fid']);
        // MORE FOTOS
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.*, u.user_name, u.user_activo, p.user_pais, p.user_sexo, u.user_rango, r.r_name, r.r_color, r.r_image FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user LEFT JOIN u_perfil AS p ON p.user_id = u.user_id LEFT JOIN u_rangos AS r ON u.user_rango = r.rango_id WHERE f.foto_id = \''.(int)$fid.'\' '.($tsUser->is_admod || $tsUser->permisos['moacp'] ? '' : 'AND f.f_status = \'0\' AND u.user_activo = \'1\'').' LIMIT 1');
        $data['foto'] = db_exec('fetch_assoc', $query);

        $data['foto']['user_avatar'] = $tsCore->getAvatar($data['foto']['f_user']);
        
        $q1 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(cid) AS cf FROM f_comentarios WHERE c_user = \''.$data['foto']['f_user'].'\''));
		$q2 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(foto_id) AS f FROM f_fotos WHERE f_user = \''.$data['foto']['f_user'].'\' && f_status = \'0\''));
        ...
        // FOLLOW
		$data['foto']['follow'] = db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT `follow_id` FROM `u_follows` WHERE f_user = \''.$tsUser->uid.'\' AND f_id = \''.(int)$data['foto']['f_user'].'\' AND f_type = \'1\' LIMIT 1'));
        
        // SEGUIDORES
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.f_id, p.foto_id, p.f_title, p.f_url, u.user_name FROM u_follows AS f LEFT JOIN f_fotos AS p ON f.f_id = p.f_user LEFT JOIN u_miembros AS u ON p.f_user = u.user_id WHERE f.f_user = \''.$data['foto']['f_user'].'\' AND f.f_type = \'1\' AND p.f_last = \'1\' LIMIT 6');
        $data['amigos'] = result_array($query);
        
        // ULTIMAS FOTOS
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.foto_id, f.f_title, f.f_date, f.f_status, f.f_url, u.user_name, u.user_activo FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user WHERE f.f_user = \''.$data['foto']['f_user'].'\' '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'AND f.f_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY f.foto_id DESC LIMIT 6');
		...
		$data['visitas'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT v.*, u.user_id, u.user_name FROM w_visitas AS v LEFT JOIN u_miembros AS u ON v.user = u.user_id WHERE v.for = \''.(int)$fid.'\' && v.type = \'3\' && v.user > 0 ORDER BY v.date DESC LIMIT 15'));
		}
        // UPDATES
		$visitado = db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT id FROM `w_visitas` WHERE `for` = \''.(int)$fid.'\' && `type` = \'3\' && '.($tsUser->is_member ? '(`user` = \''.$tsUser->uid.'\' OR `ip` LIKE \''.$_SERVER['REMOTE_ADDR'].'\')' : '`ip` LIKE \''.$_SERVER['REMOTE_ADDR'].'\'').' LIMIT 0,100'));
		if($tsUser->is_member && $visitado == 0) {
			db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO w_visitas (`user`, `for`, `type`, `date`, `ip`) VALUES (\''.$tsUser->uid.'\', \''.(int)$fid.'\', \'3\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
			db_exec([__FILE__, __LINE__], 'query', 'UPDATE f_fotos SET f_hits = f_hits + 1 WHERE foto_id = \''.(int)$fid.'\' AND f_user != \''.$tsUser->uid.'\'');		
		}else{
		db_exec([__FILE__, __LINE__], 'query', 'UPDATE `w_visitas` SET `date` = \''.time().'\', ip = \''.$_SERVER['REMOTE_ADDR'].'\' WHERE `for` = \''.(int)$post_id.'\' && `type` = \'3\'');
		}
		if($tsCore->settings['c_hits_guest'] == 1 && !$tsUser->is_member && !$visitado) {
			db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO w_visitas (`user`, `for`, `type`, `date`, `ip`) VALUES (\'0\', \''.(int)$fid.'\', \'3\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
			db_exec([__FILE__, __LINE__], 'query', 'UPDATE f_fotos SET f_hits = f_hits + 1 WHERE foto_id = \''.(int)$fid.'\'');
		}
		//
		$this->DarMedalla($fid);
		//
	...
	*/
	function DarMedalla($fid){
		//
		$data = db_exec('fetch_assoc', $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT foto_id, f_user, f_votos_pos, f_votos_neg, f_hits FROM f_fotos WHERE foto_id = \''.(int)$fid.'\' LIMIT 1'));
		//
        $q1 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(cid) AS a FROM f_comentarios WHERE c_foto_id = \''.(int)$fid.'\''));
        $q2 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(wm.medal_id) AS m FROM w_medallas AS wm LEFT JOIN w_medallas_assign AS wma ON wm.medal_id = wma.medal_id WHERE wm.m_type = \'3\' AND wma.medal_for = \''.(int)$fid.'\''));
		// MEDALLAS
		$datamedal = result_array($query = db_exec([__FILE__, __LINE__], 'query', 'SELECT * FROM w_medallas WHERE m_type = \'3\' ORDER BY m_cant DESC'));
		...
		if(!empty($newmedalla)){
		  $q3 = db_exec('fetch_row', db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(id) AS w FROM w_medallas_assign WHERE medal_id = \''.(int)$newmedalla.'\' AND medal_for = \''.(int)$fid.'\''));
		if(!$q3[0]){
		db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `w_medallas_assign` (`medal_id`, `medal_for`, `medal_date`, `medal_ip`) VALUES (\''.(int)$newmedalla.'\', \''.(int)$fid.'\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
		db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO u_monitor (user_id, obj_uno, obj_dos, not_type, not_date) VALUES (\''.(int)$data['f_user'].'\', \''.(int)$newmedalla.'\', \''.(int)$fid.'\', \'17\', \''.time().'\')');
    ...
    */
    function votarFoto(){
		...
		if($tsUser->is_member){
			...
			if(!$is_mypost){
				// YA LO VOTE?
				$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT vid FROM f_votos WHERE v_foto_id = \''.(int)$fid.'\'  AND v_user = \''.$tsUser->uid.'\' LIMIT 1');
				$votado = db_exec('num_rows', $query);
				
				if(empty($votado)){
					// SUMAR VOTO
					db_exec([__FILE__, __LINE__], 'query', 'UPDATE f_fotos SET '.$voto.' WHERE foto_id = \''.(int)$fid.'\'');
					// INSERTAR EN TABLA
					if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `f_votos` (`v_foto_id`, `v_user`, `v_type`, `v_date`) VALUES (\''.(int)$fid.'\', \''.$tsUser->uid.'\', \''.$type.'\', \''.time().'\')')) return '1: Votado';
					//
    ...
    */
    function newComentario(){
        ...
        /* DE QUIEN ES LA FOTO */
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f_user, f_closed FROM f_fotos WHERE foto_id = \''.(int)$fid.'\' LIMIT 1');
		$data = db_exec('fetch_assoc', $query);
        ...
        if($data['f_user']){
            if($data['f_closed'] != 1 || $data['f_user'] == $tsUser->uid){
				...
				$_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
                if(!filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)) { die('0: Su ip no se pudo validar.'); }
				if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO f_comentarios (c_foto_id, c_user, c_date, c_body, c_ip) VALUES (\''.(int)$fid.'\', \''.$tsUser->uid.'\', \''.$fecha.'\', \''.$comentario.'\', \''.$_SERVER['REMOTE_ADDR'].'\')')) {
        		  	$cid = db_exec('insert_id');
                    // ESTADÍSTICAS
                    db_exec([__FILE__, __LINE__], 'query', 'UPDATE `w_stats` SET `stats_foto_comments` = stats_foto_comments + \'1\' WHERE `stats_no` = \'1\'');
    ...
    */
    function delComentario(){
        ...
        // DATOS
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT c.cid, c.c_user, f.foto_id, f.f_user FROM f_comentarios AS c LEFT JOIN f_fotos AS f ON c.c_foto_id = f.foto_id WHERE c.cid = \''.(int)$cid.'\' LIMIT 1');
        $data = db_exec('fetch_assoc', $query);
        ...
        if(!empty($data['cid'])){
			...
			if(db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM f_comentarios WHERE cid = \''.(int)$cid.'\'')){
			     db_exec([__FILE__, __LINE__], 'query', 'UPDATE `w_stats` SET `stats_foto_comments` = stats_foto_comments - \'1\' WHERE `stats_no` = \'1\'');
                    return '1: Borrado';
                }
    ...
    # SEO FOR POSTS
   function SeoFotos($foto) {
      global $tsCore;
      $seo = db_exec([__FILE__, __LINE__], 'query', "SELECT f.foto_id, f.f_title, f.f_url, f.f_description, f.f_date, u.user_name FROM f_fotos AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.foto_id = {$foto}");
      $data = db_exec('fetch_assoc', $seo);
      // META
      $data['Url'] = $tsCore->settings['url'].'/fotos/'.$data['user_name'].'/'.$data['foto_id'].'/'.$tsCore->setSEO($data['f_title']).'.html';
      $data['Seo'] = $tsCore->truncate($data['f_description'], 180);
