 ...
 * @author  PHPost Team
 */
require_once ROUTEEXTRAS . "datos.php";
class tsCuenta {
    ...
    */
	public function loadPerfil($user_id = 0){
		...
		//
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.*, u.user_avatar, u.user_socials, u.user_registro, u.user_lastactive FROM u_perfil AS p LEFT JOIN u_miembros AS u ON p.user_id = u.user_id WHERE p.user_id = \''.(int)$user_id.'\' LIMIT 1');
	...
	*/
	function loadHeadInfo(int $user_id = 0){
      ...
      // Redes Sociales
		...
		}else db_exec([__FILE__, __LINE__], 'query', 'UPDATE `w_visitas` SET `date` = \''.time().'\', ip = \''.$_SERVER['REMOTE_ADDR'].'\' WHERE `for` = \''.(int)$post_id.'\' && `type` = \'1\'');
	...
	*/
	function loadGeneral($user_id){
		global $tsCore;
		# SIGNO
		$query = db_exec([__FILE__, __LINE__], 'query', "SELECT user_dia, user_mes FROM u_perfil WHERE user_id = {$user_id}");
      ...
      $data['m_total'] = safe_count($data['medallas']);
		...
		// SEGUIDORES
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.follow_id, u.user_id, u.user_name FROM u_follows AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.f_id = \''.(int)$user_id.'\' && f.f_type = \'1\' && u.user_activo = \'1\' && u.user_baneado = \'0\' ORDER BY f.f_date DESC LIMIT 6');
      $data['segs']['data'] = result_array($query);
      $data['segs']['total'] = safe_count($data['segs']['data']);
        
		// SIGUIENDO
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.follow_id, u.user_id, u.user_name FROM u_follows AS f LEFT JOIN u_miembros AS u ON f.f_id = u.user_id WHERE f.f_user = \''.(int)$user_id.'\' AND f.f_type = \'1\' && u.user_activo = \'1\' && u.user_baneado = \'0\' ORDER BY f.f_date DESC LIMIT 6');
    ...
    */
    function loadPosts($user_id){
        global $tsUser;
        //
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_id, p.post_title, p.post_puntos, c.c_seo, c.c_nombre FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' AND p.post_user = \''.(int)$user_id.'\' ORDER BY p.post_date DESC LIMIT 18');
   ...
   */
   function cambiarPortada($usuario, $portada) {
   	...
   	if(db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_perfil SET p_portada = \''.$IMG.'\' WHERE user_id = \''.(int)$usuario.'\'')) return '1: Se cargo portada correctamente.';
   	else '0: Hubo un error, vuelva a intentarlo.';
   }

   public function getSettingsUser() {
   	$tsUser = new tsUser;
		$query = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT setting_mode, setting_position, setting_repeat, setting_size, setting_type, setting_id, setting_width, setting_height FROM u_settings WHERE user_id = " .  (int)$tsUser->uid));
		return $query;
	}
	/*
		savePerfil()
