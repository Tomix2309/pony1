...
class tsBuscador {
   ...
   public function getQuery() {
      ...
      if($e != 'fotos') {
         ...
         // PAGINAS
         $query = db_exec([__FILE__, __LINE__], 'query', "SELECT COUNT(f.foto_id) AS total FROM f_fotos AS f WHERE f.f_status = 0 {$w_autor} {$w_search} ORDER BY f.f_date");
         $total = db_exec('fetch_assoc', $query);
         $total = $total['total'];

         $data['pages'] = $tsCore->getPagination($total, 12);
         //
         $query = db_exec([__FILE__, __LINE__], 'query', "SELECT f.foto_id, f.f_user, f.f_title, f.f_url, f.f_description, f.f_date, f.f_votos_pos, f.f_votos_neg, u.user_name FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user WHERE f.f_status = 0 {$w_autor} {$w_search} ORDER BY f.f_date DESC LIMIT " . $data['pages']['limit']);
         $data['data'] = result_array($query);
      }
      // ACTUALES
   ...
   function BuscarUsuarios() {
      ...
      # Creamos la sentencia
	   $sentencia = ($all == 'todos') ? "u.user_id, u.user_name, u.user_activo, u.user_baneado FROM u_miembros AS u WHERE {$MiId}" : "DISTINCT u.user_id, u.user_name, u.user_activo, u.user_baneado FROM u_miembros AS u WHERE {$MiId} AND u.user_name LIKE '%$usuarios%'";
		...
		$query = db_exec([__FILE__, __LINE__], 'query', "SELECT {$sentencia} ORDER BY u.user_id ASC");
      # Mostramos
