	  ...
	  */
	  public function getMyPosts(){
		  ...
		  if(is_array($cat_ids)){
				...
				//
				$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_id, p.post_category, p.post_title, p.post_date, p.post_puntos, p.post_private, u.user_name, c.c_nombre, c.c_seo FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' AND '.$where.' ORDER BY p.post_id DESC LIMIT '.$pages['limit'].'');
	...
	function getLastPosts($type = 'visited'){
		...
		// LO HAGO ASI PARA ORDENAR SIN NECESITAR OTRA VARIABLE
		  foreach($visited as $key => $id){
				$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_id, p.post_user, p.post_category, p.post_title, p.post_date, p.post_puntos, p.post_private, u.user_name, c.c_nombre, c.c_seo FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = 0 AND p.post_id = '.$id.' LIMIT 1');
				$data[] = db_exec('fetch_assoc', $query);
	  ...
	  */
	  public function getFavorites(){
		  ...
		  //
		$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.fav_id, f.fav_date, p.post_id, p.post_title, p.post_date, p.post_puntos, p.post_category, p.post_private, COUNT(p_c.c_post_id) as post_comments,  c.c_nombre, c.c_seo FROM p_favoritos AS f LEFT JOIN p_posts AS p ON p.post_id = f.fav_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN p_comentarios AS p_c ON p.post_id = p_c.c_post_id && p_c.c_status = \'0\' WHERE f.fav_user = \''.$tsUser->uid.'\' && p.post_status = \'0\' GROUP BY c_post_id ORDER BY f.fav_date DESC LIMIT '.$pages['limit']);
	  ...
	  */
	  public function getFotos(){
		  // FOTOS
		include(ROUTECLASS."c.fotos.php");
		$tsFotos = new tsFotos();
