...
require ROUTECLASS . "c.core-extends.php";

class tsCore extends tsCoreExtends {
	...
	public function __construct() {
		...
		$this->settings['domain'] 		= self::getDomain();
		$this->settings['categorias'] = $this->getCategorias();
      $this->settings['default'] 	= $this->settings['url'].'/public/themes/default';
      ...
      $this->settings['css'] 			= $this->settings['tema']['t_url'].'/css';
		...
		// FILES
		$this->settings['private'] 	= $this->settings['url'].'/private';
		$this->settings['public'] 		= $this->settings['url'].'/public';
		$this->settings['assets'] 		= $this->settings['public'].'/assets';
	...
	*/
	...
	function getAds(){
		return db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', 'SELECT * FROM w_ads'));
	}
	...
	/* CHANGE NICKS */
	function getNicks() {
      $datos = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', 'SELECT (SELECT count(id) FROM u_nicks WHERE estado = \'0\') as nicks'));
	...
	*/
	public function getTema() {
      ...
      $data['t_url'] = $this->settings['url'] . '/public/themes/' . $data['t_path'];
		//
		return $data;
	}
	/*
   ...
   public function getDomain() {
      $domain = explode('/', str_replace(parent::getProtocol(), '', $this->settings['url']));
	...
	public function currentUrl(){
		$current_url = parent::getProtocol() . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
	...
	*/
	function parseBBCode($bbcode, $type = 'normal', $act = 0, $title = null, $menciones = false) {
      // Class BBCode
      include_once(ROUTEEXTRAS . 'bbcode.inc.php');
      $parser = new BBCode();
