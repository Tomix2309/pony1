...
class tsJson {
	...
	*/
	public function getContentJson(string $type = '', int $id = 0) {
		$filename_base = ROUTECONFIGURACION . "config-example-$type.json";
		$filename_copy = ROUTECONFIGURACION . ($id === 0 ? "settings.json" : "config-$id.json");
	...
	*/
	public function getAddInfo() {
		...
		$backgroundroute = ROUTEDESCARGAS . $backgroundname;
	...
	*/
	public function save_json(string $param = '') {
		...
		$rsp = (file_put_contents(ROUTECONFIGURACION . "settings.json", json_encode($admin, JSON_PRETTY_PRINT))) ? '1: Los cambios fuerón aplicados!' :  '0: Los datos no se guardaron.';
		return $rsp;
	}
	...
	*/
	public function save_mode() {
		...
		$rsp = (file_put_contents(ROUTECONFIGURACION . "$in.json", json_encode($admin, JSON_PRETTY_PRINT))) ? '1: Los cambios fuerón aplicados!' :  '0: Los datos no se guardaron.';
		return $rsp;
	}
	...
	*/
	public function saveInfoPortada() {
		...
		if($tsUser->uid == $id) {
			...
			$file_user = ROUTECONFIGURACION . "config-{$sql['user_id']}.json";
			$j = [];
			if ( file_exists($file_user) ){
				$j = (array)json_decode(file_get_contents($file_user));
			} else {
				# Si no existe creamos uno! No preguntamos.
				$fichero = ROUTECONFIGURACION . "config-example-user.json";
				$nuevo_fichero = ROUTECONFIGURACION . "config-{$sql['user_id']}.json";
