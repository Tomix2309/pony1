...
class tsAdmin {
   ...
   */
   ...
   public function getNoticia() {
      ...
      $query = db_exec([__FILE__, __LINE__], 'query', "SELECT `not_id`, `not_body`, `not_date`, `not_active` FROM w_noticias WHERE not_id = '{$not_id}' LIMIT 1");
   ...
   public function newNoticia() {
      ...
      $time = time();
      if (!empty($not_body)) {
         if (db_exec([__FILE__, __LINE__], 'query', "INSERT INTO `w_noticias` (`not_body`, `not_autor`, `not_date`, `not_active`) VALUES ('{$not_body}', '{$tsUser->uid }', '{$time}', '{$not_active}')")) return true;
      }
      return false;
   }
   /*
   ...
   */
   ...
   public function getTema($var)
   {
      ...
      //
      $query = ($var == 'true') ? db_exec([__FILE__, __LINE__], 'query', 'SELECT t.t_name, t.t_copy, c.tema_id FROM w_temas AS t LEFT JOIN w_configuracion AS c ON t.tid = c.tema_id LIMIT 1') : db_exec([__FILE__, __LINE__], 'query', 'SELECT `tid`, `t_name`, `t_url`, `t_path` FROM `w_temas` WHERE tid = \'' . (int) $tema_id . '\' LIMIT 1');
      $data  = db_exec('fetch_assoc', $query);
      //
      return $data;
   }
   ...
   public function saveTema()
   {
      ...
      //
      if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE `w_temas` SET t_url = \'' . $t['url'] . '\', t_path = \'' . $t['path'] . '\' WHERE tid = \'' . (int) $tema_id . '\'')) {
         return true;
      } else {
         return false;
      }
   ...
   public function getTemaModal()
   {
      ...
      $tema_id = $tsCore->setSecure($_GET['tid']);
      //
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT `tid`, `t_name`, `t_url`, `t_path` FROM `w_temas` WHERE tid = \'' . (int) $tema_id . '\' LIMIT 1');
      $data  = db_exec('fetch_assoc', $query);
      //
      return $data;
   }
   ...
   public function newTema()
   {
      ...
      // NUEVO
      if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `w_temas` (`t_name`, `t_url`, `t_path`, `t_copy`) VALUES (\'' . $tsCore->setSecure($temadb['nombre']) . '\', \'' . $tsCore->setSecure($temadb['url']) . '\', \'' . $tsCore->setSecure($tema_path) . '\', \'' . $tsCore->setSecure($temadb['copy']) . '\')')) {
         return 1;
      } else {
   ...
   */
   public function saveOrden() {
      ...
      foreach ($subcats as $key => $cid) {
         if (!empty($cid)) {
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_categorias` SET c_orden = \'' . (int) $orden . '\' WHERE cid = \'' . (int) $cid . '\'');
            $orden++;
         }
      }
   }
   ...
   public function getCat()
   {
      ...
      $cid = intval($_GET['cid']);
      //
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT cid, c_orden, c_nombre, c_seo FROM p_categorias WHERE cid = \'' . (int) $cid . '\' LIMIT 1');
      $data = db_exec('fetch_assoc', $query);
   ...
   public function saveCat() {
      ...
      if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_categorias` SET c_nombre = \'' . $tsCore->setSecure($c_nombre) . '\', c_seo = \'' . $tsCore->setSecure($tsCore->setSEO($c_nombre, true)) . '\' WHERE cid = \'' . (int) $cid . '\'')) {
         return true;
      }
   ...
   public function MoveCat() {
      if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_posts` SET post_category = \'' . (int) $_POST['newcid'] . '\' WHERE post_category = \'' . (int) $_POST['oldcid'] . '\'')) {
         return true;
      }
   ...
   public function newCat()
   {
      ...
      $orden = $orden['total'] + 1;
      // INSERTS
      if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `p_categorias` (`c_orden`, `c_nombre`, `c_seo`) VALUES (\'' . $orden . '\', \'' . $c_nombre . '\',\'' . $tsCore->setSEO($c_nombre, true) . '\')')) {
         return true;
      }
   ...
   */
   ...
   public function deleteContent($user_id) {
      ...
      if (db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT user_id FROM u_miembros WHERE user_id = \'' . $tsUser->uid . '\' && user_password = \'' . md5(md5($_POST['password']) . strtolower($tsUser->nick)) . '\''))) {
         $c = $_POST['bocuenta'];
         # posts
         if ($_POST['boposts'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM p_posts WHERE post_user = " . $user_id);
         # comentarios en posts
         if ($_POST['bocomposts'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM p_comentarios WHERE c_user = " . $user_id);
         # fotos
         if ($_POST['bofotos'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM f_fotos WHERE f_user = " . $user_id);
         # comentarios en fotos
         if ($_POST['bocomfotos'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM f_comentarios WHERE c_user = " . $user_id);
         # archivos
         if ($_POST['bofiles'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM a_files WHERE f_user = " . $user_id);
         # comentarios en archivos
         if ($_POST['bocomfiles'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM a_comentarios WHERE com_user = " . $user_id);
         # estados
         if ($_POST['boestados'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_muro WHERE p_user_pub = " . $user_id);
         # comentarios en estados
         if ($_POST['bocomestados'] || $c)
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_muro_comentarios WHERE c_user = " . $user_id);
         # likes
         if ($_POST['bolikes'] || $c)
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_muro_likes WHERE user_id = " . $user_id);
         # seguidores
         if ($_POST['boseguidores'] || $c)
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_follows WHERE f_id = {$user_id} && f_type = 1");
         # siguiendo
         if ($_POST['bosiguiendo'] || $c)
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_follows WHERE f_user = {$user_id} && f_type = 1");
         # favoritos
         if ($_POST['bofavoritos'] || $c) {
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM p_favoritos WHERE fav_user = " . $user_id);
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM a_favoritos WHERE fav_user = " . $user_id);
         }
         # votos en posts
         if ($_POST['bovotosposts'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM p_votos WHERE tuser = " . $user_id);
         # votos en fotos
         if ($_POST['bovotosfotos'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM f_votos WHERE v_user = " . $user_id);
         # actividades
         if ($_POST['boactividad'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_actividad WHERE user_id = " . $user_id);
         # avisos
         if ($_POST['boavisos'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_avisos WHERE user_id = " . $user_id);
         # bloqueos
         if ($_POST['bobloqueos'] || $c)
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_bloqueos WHERE b_user = " . $user_id);
         # mensajes
         if ($_POST['bomensajes'] || $c) {
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_mensajes WHERE mp_from = " . $user_id);
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_respuestas WHERE mr_from = " . $user_id);
         }
         # sesiones
         if ($_POST['bosesiones'] || $c)
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM u_sessions WHERE session_user_id = " . $user_id);
         # visitas
         if ($_POST['bovisitas'] || $c) 
            db_exec([__FILE__, __LINE__], 'query', "DELETE FROM w_visitas WHERE user = " . $user_id);
   ...
   public function ChangeNick_o_no()
   {
      ...
      // <--
      include ROUTEGENERAL . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'class' .
   ...
   public function GetAdminPosts() {
      ...
      //
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_id, u.user_name, c.c_nombre, c.c_seo, p.* FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_id > \'0\' ORDER BY ' .
         $order . ' ' . ($_GET['m'] == 'a' ? 'ASC' : 'DESC') . ' LIMIT ' . $limit);
      //
      $data['data'] = result_array($query);
   ...
   public function GetAdminFiles() {
      ...
      //
      $query = db_exec([__FILE__, __LINE__], 'query', "SELECT u.user_id, u.user_name, f.* FROM a_files AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.file_id > 0 ORDER BY {$order} {$ascdesc} LIMIT ". $limit);
      //
      $data['data'] = result_array($query);

      // PAGINAS
      $query       = db_exec([__FILE__, __LINE__], 'query', 'SELECT COUNT(*) FROM a_files WHERE file_id > \'0\'');
      list($total) = db_exec('fetch_row', $query);
   ...
   public function newTheme()
   {
      ...
      if ($_FILES["zip_file"]["name"]) {
         ...
         if ($zip->open($ruta) === true) {
            ...
            db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `w_temas` (`t_name`, `t_url`, `t_path`, `t_copy`) VALUES (\'' . $tsCore->setSecure($Nombre) . '\', \'' . $tsCore->setSecure($UrlTheme) . '\', \'' . $tsCore->setSecure($Nombre) . '\', \'' . $tsCore->setSecure($Autor) . '\')');
         } else {
