...
class tsPosts {
   ...
   */
   ...
   function newPost()
   {
      ...
      if ($tsUser->is_admod || $tsUser->permisos['gopp']) {
         ...
         if ($d[0]) {
            die('No se puede agregar el post. C&oacute;digo de error [#0aP]');
         }
         ...
         if ($tsUser->info['user_lastpost'] < (time() - $antiflood)) {
            ...
            if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `p_posts` (post_user, post_category, post_title, post_portada, post_body, post_date, post_tags, post_ip, post_private, post_block_comments, post_sponsored, post_sticky, post_smileys, post_visitantes, post_status) VALUES (\'' . $tsUser->uid . '\', \'' . (int) $postData['category'] . '\', \'' . $postData['title'] . '\', \'' . $postData['portada'] . '\', \'' . $postData['body'] . '\', \'' . $postData['date'] . '\', \'' . $postData['tags'] . '\', \'' . $_SERVER['REMOTE_ADDR'] . '\', \'' . (int) $postData['private'] . '\', \'' . (int) $postData['block_comments'] . '\', \'' . (int) $postData['sponsored'] . '\', \'' . (int) $postData['sticky'] . '\', \'' . (int) $postData['smileys'] . '\', \'' . (int) $postData['visitantes'] . '\', ' . (!$tsUser->is_admod && ($tsCore->settings['c_desapprove_post'] == 1 || $tsUser->permisos['gorpap'] == true) ? '\'3\'' : '\'0\'') . ')')) {
               ...
               if (!$tsUser->is_admod && ($tsCore->settings['c_desapprove_post'] == 1 || $tsUser->permisos['gorpap'] == true)) {
                  db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `w_historial` (`pofid`, `action`, `type`, `mod`, `reason`, `date`, `mod_ip`) VALUES (\'' . (int) $postID . '\', \'3\', \'1\', \'' . $tsUser->uid . '\', \'Revisi&oacute;n al publicar\', \'' . time() . '\', \'' . $_SERVER['REMOTE_ADDR'] . '\')');
               }
   ...
   /*
      getCatData() :: OBTENER DATOS DE UNA CATEGORIA
   */
   function getCatData() {
      global $tsCore;
      //
   ...
   function getLastPosts($category = null, $subcateg = null, $sticky = false)
   {
      ...
      //$nop = 'AND p.post_category != \'18\' && p.post_category != \'32\'';
      // Stickys
      if ($sticky) {
   ...
   # Haciendo menciones a usuario y creando enlaces
   function getLinkPost($urlUserLink) {
      ...
      foreach ($menciones as $k => $user) {
         ...
         if(!empty($uid)) {
            ...
            $data = str_replace($user, $reemplazar, $urlUserLink);
         }
      }
   ...
   function getPost()
   {
      ...
      if (empty($postData['post_id'])) {
         ...
         if (!empty($tsDraft['b_title'])) {
            return array('deleted', 'Oops! Este post no existe o fue eliminado.');
         } else {
            return array('deleted', 'Oops! El post fue eliminado!');
         }

      } elseif ($postData['post_status'] == 1 && (!$tsUser->is_admod && $tsUser->permisos['moacp'] == false)) {
         return array('denunciado', 'Oops! El Post se encuentra en revisi&oacute;n por acumulaci&oacute;n de denuncias.');
      } elseif ($postData['post_status'] == 2 && (!$tsUser->is_admod && $tsUser->permisos['morp'] == false)) {
         return array('deleted', 'Oops! El post fue eliminado!');
      } elseif ($postData['post_status'] == 3 && (!$tsUser->is_admod && $tsUser->permisos['mocp'] == false)) {
         return array('denunciado', 'Oops! El Post se encuentra en revisi&oacute;n, a la espera de su publicaci&oacute;n.');
      } elseif (!empty($postData['post_private']) && empty($tsUser->is_member)) {
         return array('privado', $postData['post_title']);
      }
      ...
      if ($postData['post_cache'] < time() - ($tsCore->settings['c_stats_cache'] * 60)) {
         ...
         // PORTADA DEL POST
         $postData['post_cover'] = $tsCore->extraer_img($postData['post_body']);
      ...
      // BBCode
      $postData['post_body']  = $tsCore->parseBadWords($postData['post_smileys'] == 0 ? $tsCore->parseBBCode($postData['post_body'], 'normal', 1, $postData['post_title'], true) : $tsCore->parseBBCode($postData['post_body'], 'firma'), true);
      $postData['user_firma'] = $tsCore->parseBadWords($tsCore->parseBBCodeFirma($postData['user_firma']), true);
      $postData['post_portada'] = !empty($postData['post_portada']) ? $postData['post_portada'] : $tsCore->extraer_img($postData['post_body']);
      // Enlaces automatico
      ...
      // CALIFICACIONES
      $query = db_exec([__FILE__, __LINE__], 'query', "SELECT * FROM p_votos WHERE tid = '{$postData['post_id']}'");
      $postData['total_people'] = result_array($query);
      // CANTIDAD DE PERSONAS QUE HAN CALIFICADO EL POST
      $postData['star5'] = 0;
      $postData['star4'] = 0;
      $postData['star3'] = 0;
      $postData['star2'] = 0;
      $postData['star1'] = 0;
      foreach ($postData['total_people'] as $key => $cantidad) {
         if($postData['total_people'][$key]["cant"] == 5) $postData['star5'] += round($postData['total_people'][$key]["cant"] * 10) / 100;
         if($postData['total_people'][$key]["cant"] == 4) $postData['star4'] += round($postData['total_people'][$key]["cant"] * 10) / 100;
         if($postData['total_people'][$key]["cant"] == 3) $postData['star3'] += round($postData['total_people'][$key]["cant"] * 10) / 100;
         if($postData['total_people'][$key]["cant"] == 2) $postData['star2'] += round($postData['total_people'][$key]["cant"] * 10) / 100;
         if($postData['total_people'][$key]["cant"] == 1) $postData['star1'] += round($postData['total_people'][$key]["cant"] * 10) / 100;
      }
      $postData['total_people'] = empty($postData['total_people']) ? 0 : count($postData['total_people']);
      // FORMULA PARA CALCULAR 
      $postData['StarTotal'] = round($postData['post_puntos'] * 10) / 100;
      ...
      if ($tsCore->settings['c_allow_portal']) {
         ...
         if ($total > 10) array_splice($visited, 0, 1); // HACK
         //
   ...
   function getAutor($user_id)
   {
      ...
      $data['avatar'] = $tsCore->getAvatar($user_id);
      ...
      if ($data['user_lastactive'] > $is_online) {
         $data['status'] = array('t' => 'Usuario Online', 'css' => 'online');
      } elseif ($data['user_lastactive'] > $is_inactive) {
         $data['status'] = array('t' => 'Usuario Inactivo', 'css' => 'inactive');
      } else {
         $data['status'] = array('t' => 'Usuario Offline', 'css' => 'offline');
      }

      // PAIS
      include ROUTEEXTRAS . "datos.php"; // Fix 10/06/2013
   ...
   */
   function deletePost()
   {
      ...
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT post_id, post_title, post_portada, post_user, post_body, post_category FROM p_posts WHERE post_id = \'' . (int) $post_id . '\' AND post_user = \'' . $tsUser->uid . '\'');
      $data  = db_exec('fetch_assoc', $query);
      ...
      if (!empty($data['post_id']) || !empty($tsUser->is_admod)) {
         ...
         if (db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM p_posts WHERE post_id = \'' . (int) $post_id . '\'')) {
            if (db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM p_comentarios WHERE c_post_id = \'' . (int) $post_id . '\'')) {
               if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `p_borradores` (b_user, b_date, b_title, b_portada, b_body, b_tags, b_category, b_status, b_causa) VALUES (\'' . $tsUser->uid . '\', \'' . time() . '\', \'' . $tsCore->setSecure($data['post_title']) . '\', \'' . $tsCore->setSecure($data['post_portada']) . '\', \'' . $tsCore->setSecure($data['post_body']) . '\', \'\', \'' . $data['post_category'] . '\', \'2\', \'\')')) {
                  return "1: El post fue eliminado satisfactoriamente.";
               }

            }
         } else {
            if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE p_posts SET post_status = \'2\' WHERE post_id = \'' . (int) $post_id . '\'')) {
   ...
   function getRelated($tags)
   {
      ...
      //
      $query = db_exec([__FILE__, __LINE__], 'query', "SELECT DISTINCT p.post_id, p.post_title, p.post_body, p.post_portada, p.post_category, p.post_private, c.c_seo FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE MATCH (post_tags) AGAINST ('$tags' IN BOOLEAN MODE) AND p.post_status = 0 AND post_sticky = 0 ORDER BY rand() LIMIT 0,10");
      //
      $data = result_array($query);
      foreach ($data as $id => $row) {
         $data[$id]['post_cover'] = $tsCore->extraer_img($data[$id]['post_body']);
      }
   ...
   function newComentario()
   {
      ...
      if ($data['post_user']) {
         if ($data['post_block_comments'] != 1 || $data['post_user'] == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos['mocepc']) {
            if (empty($tsUser->is_admod) && $tsUser->permisos['gopcp'] == false) {
            ...
            if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `p_comentarios` (`c_post_id`, `c_user`, `c_date`, `c_body`, `c_ip`) VALUES (\'' . (int) $post_id . '\', \'' . $tsUser->uid . '\', \'' . $fecha . '\', \'' . $comentario . '\', \'' . $_SERVER['REMOTE_ADDR'] . '\')')) {
               ...
               if (!empty($most_resp)) {
                  return array($cid, $tsCore->parseBadWords($tsCore->parseBBCode($comentario), true), $comentario, $fecha, $_POST['auser'], '', $_SERVER['REMOTE_ADDR']);
               } else {
      ...
      if ($tsUser->is_admod || $tsUser->permisos['moaydcp']) {
         ...
         if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE p_comentarios SET c_status = ' . ($data['c_status'] == 1 ? '\'0\'' : '\'1\'') . ' WHERE cid = \'' . (int) $_POST['comid'] . '\'')) {
            if ($data['c_status'] == 1) {
               ...
               return '1: El comentario fue ocultado.';
            }
   ...
   */
   function votarPost() {
      ...
      if ($tsUser->is_admod || $tsUser->permisos['godp']) {
         ...
         if ($tsUser->is_admod != 1) {
            if (db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT user_id FROM u_miembros WHERE user_last_ip =  \'' . $_SERVER['REMOTE_ADDR'] . '\' AND user_id != \'' . $tsUser->uid . '\'')) || db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT session_id FROM u_sessions WHERE session_ip =  \'' . $tsCore->setSecure($_SERVER['REMOTE_ADDR']) . '\' AND session_user_id != \'' . $tsUser->uid . '\''))) {
         ...
         if (!$is_mypost) {
            ...
            if (empty($votado)) {
               ...
               if ($tsCore->settings['c_allow_points'] > 0) {
                  ...
                  //TRUCO, podrás dar todos los puntos que tengas disponibles
                  $max_points = $tsUser->info['user_puntosxdar'];
               } elseif ($tsCore->settings['c_allow_points'] == '-2') {
                  //TRUCO, podrás dar todos los puntos que quieras (sin abusar ¬¬), se restarán igual, si tienes puesto mantener puntos, estarás debiendo puntos durante una temporada.
                  $max_points = 5;
               } else {
               ...
               if ($tsUser->info['user_puntosxdar'] >= $puntos) {
                  if ($puntos > 0) {
                     // Votar sin dar puntos? No, gracias.
                     ...
                     } else return '0: Voto no v&aacute;lido. No puedes dar ' . $puntos . ' puntos, s&oacute;lo se permiten ' . $max_points;
      ...
      if ($data['post_user'] != $tsUser->uid) {
         ...
         if (empty($my_favorito)) {
            if (db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO p_favoritos (fav_user, fav_post_id, fav_date) VALUES (\'' . $tsUser->uid . '\', \'' . (int) $post_id . '\', \'' . $fecha . '\')')) {
               ...
               return '0: ' . show_error('Error al ejecutar la consulta de la l&iacute;nea ' . __LINE__ . ' de ' . __FILE__ . '.', 'db');
            }
   ...
   function getFavoritos()
   {
      global $tsCore, $tsUser;
      //
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT f.fav_id, f.fav_date, p.post_id, p.post_title, p.post_date, p.post_puntos, COUNT(p_c.c_post_id) as post_comments,  c.c_nombre, c.c_seo FROM p_favoritos AS f LEFT JOIN p_posts AS p ON p.post_id = f.fav_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN p_comentarios AS p_c ON p.post_id = p_c.c_post_id && p_c.c_status = \'0\' WHERE f.fav_user = \'' . $tsUser->uid . '\' AND p.post_status = \'0\' GROUP BY c_post_id');
      $data  = result_array($query);

      //
      foreach ($data as $fav) {
         $favoritos .= '{"fav_id":' . $fav['fav_id'] . ',"post_id":' . $fav['post_id'] . ',"titulo":"' . $fav['post_title'] . '","categoria":"' . $fav['c_seo'] . '","categoria_name":"' . $fav['c_nombre'] . '","url":"' . $tsCore->settings['url'] . '/posts/' . $fav['c_seo'] . '/' . $fav['post_id'] . '/' . $tsCore->setSEO($fav['post_title']) . '.html","fecha_creado":' . $fav['post_date'] . ',"fecha_creado_formato":"' . strftime("%d\/%m\/%Y a las %H:%M:%S hs", $fav['post_date']) . '.","fecha_creado_palabras":"' . $tsCore->setHace($fav['post_date'], true) . '","fecha_guardado":' . $fav['fav_date'] . ',"fecha_guardado_formato":"' . strftime("%d\/%m\/%Y a las %H:%M:%S hs", $fav['fav_date']) . '.","fecha_guardado_palabras":"' . $tsCore->setHace($fav['fav_date'], true) . '","puntos":' . $fav['post_puntos'] . ',"comentarios":' . $fav['post_comments'] . '},';
      }
      //
      return $favoritos;
   }
   ...
   function subirRango($user_id, $post_id = false)
   {
      ...
      if (!empty($newRango) && $newRango != $data['user_rango']) {
         //
         if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE u_miembros SET user_rango = \'' . $newRango . '\' WHERE user_id = \'' . $user_id . '\' LIMIT 1')) {
            return true;
         }
   ...
   //LO MAS BUSCADO
   function getMasDestacado() {
      global $tsCore, $tsUser;
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_portada, p.post_id, p.post_title, p.post_private, p.post_sponsored, p.post_body, p.post_status, p.post_sticky, u.user_id, u.user_name, u.user_activo, u.user_baneado, c.c_seo, c.c_nombre FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\' && p.post_comments > \'0\' GROUP BY p.post_id ORDER BY p.post_comments DESC LIMIT 1');
      $data  = result_array($query);
      foreach ($data as $id => $row) {
         $data[$id]['post_portada'] = empty($row['post_portada']) ? $tsCore->extraer_img($data[$id]['post_body']) : $row['post_portada'];
      }
      return $data;
   }
   # LO MÁS LEIDO
   function loMasLeidoComentado($type) {
      global $tsCore, $tsUser;

      $OrderBy = ($type == 'leidos') ? 'p.post_hits' : 'p.post_comments';

      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_portada, p.post_id, p.post_title, p.post_private, p.post_sponsored, p.post_body, p.post_status, p.post_sticky, u.user_id, u.user_name, u.user_activo, u.user_baneado, c.c_seo, c.c_nombre FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\' GROUP BY p.post_id ORDER BY ' . $OrderBy . ' > 0 DESC LIMIT 6');
      $data  = result_array($query);
      foreach ($data as $id => $row) {
         $data[$id]['post_portada'] = empty($row['post_portada']) ? $tsCore->extraer_img($data[$id]['post_body']) : $row['post_portada'];
      }
   ...
   # OBTENER LOS TITULOS DE LOS POSTS ANTERIOR/SIGUIENTE/ALEATORIO
   function setNP() {
      global $tsUser, $tsCore;
      # Acción 
      $action = ($_GET['action'] == 'prev') ? '<' : '>';
      # Id del post
   ...
   function Random() {
      ...
      $q = db_exec('fetch_assoc', $query);
      return $tsCore->settings['url'] . '/posts/' . $q['c_seo'] . '/' . $q['post_id'] . '/' . $tsCore->setSEO($q['post_title']) . '.html';
   }
   function TituloAnterior() {
      $post_id = intval($_GET['post_id']) - 1;
      if ($post_id <= 0) return false;
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT post_id, post_title FROM p_posts WHERE post_id = \'' . (int) $post_id . '\' LIMIT 1');
      if (db_exec('num_rows', $query) == 0) $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT post_id, post_title FROM p_posts WHERE post_id < \'' . (int) $post_id . '\' ORDER BY post_id DESC LIMIT 1');
      $data = db_exec('fetch_assoc', $query);
   ...
   function TituloSiguente() {
      ...
      if ($post_id <= 0) return false;
      $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT post_id, post_title FROM p_posts WHERE post_id = \'' . (int) $post_id . '\' LIMIT 1');
      if (db_exec('num_rows', $query) == 0) $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT post_id, post_title FROM p_posts WHERE post_id > \'' . (int) $post_id . '\' ORDER BY post_id ASC LIMIT 1');
      $data = db_exec('fetch_assoc', $query);
      return (!empty($data) ? $data : false);
   }
   # SEO FOR POSTS
   function SeoPosts($post) {
      global $tsCore;
      $seo = db_exec([__FILE__, __LINE__], 'query', "SELECT p.post_id, p.post_title, p.post_body, p.post_user, p.post_category, p.post_date, p.post_body, p.post_portada, p.post_tags, u.user_name, c.c_seo FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_id = {$post}");
      $data = db_exec('fetch_assoc', $seo);
      // META
      $data['Url'] = $tsCore->settings['url'].'/posts/'.$data['c_seo'].'/'.$data['post_id'].'/'.$tsCore->setSEO($data['post_title']).'.html';
      $data['Seo'] = $tsCore->truncate( $tsCore->nobbcode($data['post_body']), 200);
      $data['Cover'] = $tsCore->extraer_img($data['post_body']);
