...
class tsMuro {
	 ...
	 */
	 public function getPrivacity(int $user_id = 0, string $username = '', $follow = NULL, $yfollow = NULL) {
		...
		//
		$data = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT p_configs FROM u_perfil WHERE user_id = $user_id LIMIT 1"));
	...
	*/
	public function ajaxCheck($return = false, $urlin = null){
		...
		$url = $tsCore->setSecure(urldecode($url), true);
		// VALIDAR
		if(empty($url)) return "0: El campo <strong>url</strong> es obligatorio.";
		//
		switch($type){
			# VALIDAR UNA FOTO
			case 'foto':
				...
				elseif($data[0] < $min_w || $data[1] < $min_w) return "0: Tu foto debe tener un tama&ntilde;o superior a {$min_w}x{$min_w} pixeles.";
				elseif($data[0] > $max_w || $data[1] > $max_w) return "0: Tu foto debe tener un tama&ntilde;o menor a {$max_w}x{$max_w} pixeles.";
				else return ($return == false) ? "1: <a href=\"javascript:muro.load_atta('foto', '$url', this)\" class=\"uiPhoto mx-auto\"><img class=\"image rounded\" loading=\"lazy\" src=\"$url\"/></a>" : $url;
			break;
			// VALIDAR URL
			case 'enlace':
				// VALIDAR
				...
				include ROUTEEXTRAS . 'OpenGraph.php';
				$datos = ['title', 'description', 'image'];
				$result = getOpenGraph::getUrlDataInfo($url, $datos);
				$title = $tsCore->setSecure($result['title']);
				$description = $tsCore->setSecure($result['description']);
				// VALIDAR #2
				if(!$title) return '0: La url ingresada no es una p&aacute;gina web v&aacute;lida.';
				// RETORNAMOS HTML/VALOR
				if($return == false) return "1: <div class=\"uiLink rounded d-grid\"><img src=\"{$result['image']}\" class=\"object-fit-cover rounded image\"><div class=\"p-2\"><a href=\"$url\" target=\"_blank\" class=\"d-block fw-bolder\">$title</a><small class=\"d-block\">$description</small></div></div>";
				else return [
			...
			case 'video':
				// Nuevo patron para validar la url de youtube 
				// https://youtu.be/__ID__ o https://www.youtube.com/watch?v=__ID__
    			$patron = '/(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?feature=player_embedded&v=))([^&]{11})/';
    			if(preg_match($patron, $url, $matches)) $video_id = $matches[1];
    			else return '0: La direcci&oacute;n del video no es v&aacute;lida.';
				// META TAGS
				...
				if(empty($data['title'])) return '0: La URL contiene un ID de video incorrecto o el video ha sido eliminado.';
					...
					$description = html_entity_decode(nl2br($data['description']));
					// RETORNAMOS HTML/VALORES
					if($return == false)
						return "1: <div class=\"vContent\"><lite-youtube videoid=\"$video_id\" style=\"background-image: url('https://i.ytimg.com/vi/$video_id/maxresdefault.jpg');\" params=\"controls=0&start=10&end=30&modestbranding=2&rel=0&enablejsapi=1\"></lite-youtube><div class=\"vDesc\"><a href=\"http://www.youtube.com/watch?v=$video_id\" target=\"_blank\" class=\"a_blue\">{$data['title']}</a><div style=\"margin-top:5px\">$description</div></div></div>";
					else return [
						'ID' => $video_id, 
						'title' => $tsCore->setSecure($data['title'], true), 
						'desc' => $tsCore->setSecure(substr($description, 0, 160))
					];
				}
				 //
		...
		$_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
		  ...
		  switch($type){
				...
				if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO u_muro (p_user, p_user_pub, p_body, p_date, p_type, p_ip) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$data.'\', \''.$date.'\', \'1\', \''.$_SERVER['REMOTE_ADDR'].'\') ')){
						  ...
						  $return = array('pub_id' => $pub_id, 'p_user' => $pid, 'p_user_pub' => $tsUser->uid, 'p_body' => $tsCore->parseBadWords($tsCore->parseBBCode($tsCore->setMenciones($data)), true), 'p_date' => $date, 'p_likes' => 0, 'p_type' => 1, 'likes' => array('link' => 'Me gusta'));
					 }
				break;
				 // PUBLICAR FOTO
				case 'foto':
				...
				if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO u_muro (p_user, p_user_pub, p_body, p_date, p_type, p_ip) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$data.'\', \''.$date.'\', \'3\', \''.$_SERVER['REMOTE_ADDR'].'\' ) ')){
						  $pub_id = (int)db_exec('insert_id');
						  // INSERTAR ADJUNTO
						  $secure = ['url', 'title', 'description', 'image'];
						  foreach ($secure as $value) $$value = $tsCore->setSecure($tsCore->parseBadWords($enlace[$value]), true);
						  if(db_exec([__FILE__, __LINE__], 'query', "INSERT INTO u_muro_adjuntos (pub_id, a_title, a_url, a_img, a_desc) VALUES ($pub_id, '$title', '$url', '$image', '$description') ")){
								$type = 'mlink';
								// RETORNAMOS DATOS PARA EL TEMPLATE
								$return = [
									'pub_id' => $pub_id, 
									'p_user' => $pid, 
									'p_user_pub' => $tsUser->uid, 
									'p_body' => $tsCore->setMenciones($data), 
									'p_date' => $date, 
									'p_likes' => 0, 
									'p_type' => 3, 
									'likes' => ['link' => 'Me gusta'], 
		  ...
		  if($pub['p_user'] > 0){
			...
			$_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
				if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `u_muro_comentarios` (`pub_id`, `c_user`, `c_date`, `c_body`, `c_ip`) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$date.'\', \''.$tsCore->setSecure($data, true).'\', \''.$_SERVER['REMOTE_ADDR'].'\')')){
					 ...
					 // PARA LA PANTILLA
					 return array('cid' => $cid, 'c_body' => $tsCore->parseBBCode($tsCore->parseBadWords($data, true)), 'c_date' => $date, 'c_user' => $tsUser->uid, 'c_likes' => 0, 'like' => 'Me gusta','user_name' => $tsUser->nick);
	 ...
	 */
	 function getWall($user_id, $start = 0){
		  global $tsCore;
		  // PUBLICACION
		  $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.*, u.user_id, u.user_name FROM u_muro AS p LEFT JOIN u_miembros AS u ON p.p_user_pub = u.user_id WHERE p.p_user = \''.(int)$user_id.'\' ORDER BY p.pub_id DESC LIMIT '.$start.',10');
		  while($row = db_exec('fetch_array', $query)){
				// CARGAR LIKES
				if($row['p_likes'] > 0){
					 $row['likes'] = $this->getPubExtras($row['pub_id'], 'likes', $row['p_likes']);
	 ...
	 function getPubExtras($pub_id, $type = 'likes', $likes = 0){
		  ...
		  switch($type){
				...
				case 'comments':
					 ...
					 //
					 $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT c.*, u.user_id, u.user_name FROM u_muro_comentarios AS c LEFT JOIN u_miembros AS u ON c.c_user = u.user_id WHERE c.pub_id = \''.(int)$pub_id.'\' ORDER BY c.c_date DESC '.$limit.'');
					 while($row = db_exec('fetch_array', $query)){
