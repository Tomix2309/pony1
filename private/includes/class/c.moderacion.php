    ...
    */
    function getDenuncia($type = 'posts')
    {
        ...
        switch ($type)
        {
            case 'posts':
                $d_type = 1;
                $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_id, p.post_title, p.post_status, c.c_nombre, c.c_seo, u.user_name FROM p_posts AS p LEFT JOIN p_categorias AS c ON p.post_category = c.cid LEFT JOIN u_miembros AS u ON p.post_user = u.user_id WHERE p.post_id = ' .
                    $obj . ' LIMIT 1');
                break;
            case 'fotos':
                $d_type = 4;
    ...
    function getContenido()
    {
        ...
        //
        $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_id, p.post_user, p.post_title, p.post_date, p.post_ip, u.user_name, c.c_nombre, c.c_seo FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE ' .
