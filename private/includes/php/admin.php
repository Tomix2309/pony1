...
\*********************************/
	...
	// CLASE POSTS
	include ROUTECLASS . 'c.admin.php';
	$tsAdmin = new tsAdmin();
...
\*********************************/
   ...
   /** NOTICIAS **/
	...
	// Redes sociales
    	...
    	// CLASE MEDAL
    	require_once ROUTECLASS . "c.socials.php";
	...
	# ADMIN TEMAS
		...
		# ACTION 3
		$act3 = htmlspecialchars($_GET['ttSave']);
   	$smarty->assign("ttSave",$act3);
		// VER TEMAS
		if(empty($act)){
			$smarty->assign("tsTemas",$tsAdmin->getTemas());
			$smarty->assign("tsTemaSelet",$tsAdmin->getTema('true'));
		// EDITAR TEMA
		} elseif($act == 'editar'){
			// GUARDAR
			if(!empty($_POST['save'])){
			...
			// MOSTRAR
			} else $smarty->assign("tsTema",$tsAdmin->getTema('false'));			
		// CAMBIAR TEMA
		} elseif($act == 'usar'){
			// GUARDAR
	...
	// ARCHIVOS
		...
		if(!$act) $smarty->assign("tsAdminFiles",$tsAdmin->GetAdminFiles());
	// ESTADÍSTICAS
	} elseif($action == 'stats'){
   ...
   // LISTA NEGRA
    ...
    /** MEDALLAS **/
    	...
    	// CLASE MEDAL
    	include ROUTECLASS . 'c.medals.php';
    	$tsMedal = new tsMedal();
        if(empty($act)){
        ...
        // CLASS
        include ROUTECLASS . 'c.afiliado.php';
