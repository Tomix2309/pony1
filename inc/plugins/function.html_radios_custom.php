...
function smarty_function_html_radios_output($name, $value, $output, $selected, $extra, $separator, $labels, $label_ids, $escape) {
	$_output = '';
	if (is_object($value)) {
		if (method_exists($value, '__toString')) $value = (string)$value->__toString();
		else {
	...
	if (is_object($output)) {
		if (method_exists($output, '__toString')) $output = (string)$output->__toString();
		else {
