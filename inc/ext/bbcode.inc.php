...
class BBCode {
     ...
     */
    public function setRestriction($array, $act = 0, $title = NULL) {
      $this->restriction = $array;
      $this->addBBcodes($act, $title);
    }
    ...
    private function unclosedTags() {
        ...
        $this->text = preg_replace("/\[video=(http|https)?(\:\/\/)?www\.youtube\.com\/watch\?v([A-z0-9=\-]+?)\]/i", "[video]$1$2www.youtube.com/watch?v$3[/video]", $this->text);

        $this->text = preg_replace("/\[img\=(.+?)\]/i", "[img]$1[/img]", $this->text);
        $this->text = preg_replace("/\[video\=(.+?)\]/i", "[video]$1[/video]", $this->text);
   ...
   public function addBBcodes($act = 0, $title = '') {
      ...
      $tagCodes = array(
         ...
         array('tag' => 'trailer', 'replace' => '<div class="hr_block __trailer"><i feather="play"></i> {param}</div>'),
         array('tag' => 'capturas', 'replace' => '<div class="hr_block __capturas"><i feather="image"></i> {param}</div>'),
         array('tag' => 'descargas', 'replace' => '<div class="hr_block __descargas"><i feather="download"></i> {param}</div>'),
         array('tag' => 'servidores', 'replace' => '<div class="hr_block __servidores"><i feather="coffee"></i> {param}</div>'),
         array('tag' => 'info-movie', 'replace' => '<div class="info-movie d-flex justify-content-between align-items-start">{param}</div>'),
         array('tag' => 'item', 'replace' => '<div class="info-item">{param}</div>'),
         array('tag' => 'links', 'replace' => '<div class="enlaces-download load" id="downlink">{param}</div>'),
      );
      # ESTA ES PARA LAS IMAGENES
      $tagAltImg = ($act == 1) ? trim($title) : 'Esta imagen puede contener texto';
      array_push($tagCodes, array('tag' => 'img', 'replace' => '<img src="{param}" alt="' . $tagAltImg . '" title="' . $tagAltImg . '" />'));
      ...
      if (in_array('video', $this->restriction) || !$this->restriction) $this->parser->addCodeDefinition(new Video());
    }
    ...
    public function parseMentions() {
        ...
        foreach ($users[1] as $user) {
            if (!in_array($user, $founds)) {
                ...
                if (!empty($uid)) {
                    $find = '@' . $user . ' ';
                    $replace = '<a href="' . $tsCore->settings['url'] . '/perfil/' . $user . '" class="text-dark text-capitalize font-weight-bolder" uid="' . $uid . '" title="Ir al perfil de ' . $user . '">@' . $user . '</a> ';
