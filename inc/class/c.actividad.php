...
class tsActividad {
    ...
    private function makeActividad(){
        ...
        $this->actividad = array(
            ...
            12 => array('text' => array('Coment&oacute;', 'el archivo'), 'css' => 'comment_post'),
            13 => array('text' => 'Agreg&oacute; a favoritos el post', 'css' => 'star'),
        );
    }
    /**
     * @name setActividad
    ...
    private function makeConsulta($data = NULL){
        ...
        switch($data['ac_type']){
            ...
            case 12:
            case 13:
                return 'SELECT f.file_id, f.f_user, f.f_nombre, f.f_fecha, f.f_folder, c.folder_id FROM a_files AS f LEFT JOIN a_folder_files AS c ON f.f_folder = c.folder_id LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.file_id = \''.(int)$data['obj_uno'].'\' LIMIT 1';
            break;
        }
    }
    /**
    ...
    private function makeOracion($data = NULL){
        ...
        switch($ac_type){
            ...
            case 8:
                // AVATARES
                $img_uno = '<img src="'.$tsCore->settings['url'].'/files/avatar/'.$data['user_id'].'.webp"/>';
                $img_dos = '<img src="'.$tsCore->settings['url'].'/files/avatar/'.$data['avatar'].'.webp"/>';
                // ORACION
