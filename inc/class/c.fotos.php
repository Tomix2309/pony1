...
class tsFotos {
    ...
    */
    function getLastComments(){
        ...
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT c.cid, c.c_user, f.foto_id, f.f_title, f.f_status, u.user_id, u.user_name, u.user_activo FROM f_comentarios AS c LEFT JOIN f_fotos AS f ON c.c_foto_id = f.foto_id LEFT JOIN u_miembros AS u ON f.f_user = u.user_id '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'WHERE f.f_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY c.c_date DESC LIMIT 10');
    ...
    */
    function getFoto(){
        ...
        // SEGUIDORES
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT f.f_id, p.foto_id, p.f_title, p.f_url, u.user_name FROM u_follows AS f LEFT JOIN f_fotos AS p ON f.f_id = p.f_user LEFT JOIN u_miembros AS u ON p.f_user = u.user_id WHERE f.f_user = \''.$data['foto']['f_user'].'\' AND f.f_type = \'1\' AND p.f_last = \'1\' LIMIT 6');
        $data['amigos'] = result_array($query);
        
        // ULTIMAS FOTOS
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT f.foto_id, f.f_title, f.f_date, f.f_status, f.f_url, u.user_name, u.user_activo FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user WHERE f.f_user = \''.$data['foto']['f_user'].'\' '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'AND f.f_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY f.foto_id DESC LIMIT 6');
   ...
   function SeoFotos($foto) {
      global $tsCore;
      $seo = db_exec(array(__FILE__, __LINE__), 'query', "SELECT f.foto_id, f.f_title, f.f_url, f.f_description, f.f_date, u.user_name FROM f_fotos AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.foto_id = {$foto}");
