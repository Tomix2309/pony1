...
class tsUser {
	...
	*/
	function loadUser($login = FALSE) {
      ...
      $zona = db_exec('fetch_assoc', db_exec(array(__FILE__, __LINE__), 'query', "SELECT user_pais, user_estado FROM u_perfil WHERE user_id = {$this->info['user_id']}"));   
      if(!empty($zona["user_pais"]) || !empty($zona["user_estado"]))   {
         ...
         $timezone = db_exec('fetch_assoc', db_exec(array(__FILE__, __LINE__), 'query',"SELECT zone_timezone FROM w_timezone WHERE zone_timezone LIKE '%$filter[0]%' AND zone_timezone LIKE '%$filter[1]%'"));
    ...
    function ConfigPortada($paramUser) {
        # Configuraciones del usuario
        $file_user = TS_SETTINGS . trim($paramUser) . ".json";
        if ( !file_exists($file_user) ) {
            # Si no existe creamos uno! No preguntamos.
            $fichero = TS_SETTINGS . "create_new.json";
            $nuevo_fichero = TS_SETTINGS . trim($paramUser) . ".json";
            if (!copy($fichero, $nuevo_fichero)) echo "Error al copiar $fichero...\n";
        }
        $json = json_decode(file_get_contents($file_user), true);
        # Seleccionamos el tipo unplash o pexels
        $ts = $json['portada']['type']; $h = $json['portada']['height']; $w = $json['portada']['width'];
        # Creamos la ruta de imagen, y reemplazamos por el ID
        $url = str_replace('#ID#', $json['portada']['id'], $json[$ts]['url']);
        # En el caso que sea de pexels agregamos los parámetros
        if($ts == 'pexels') {
	...
	*/
	function loginUser($username, $password, $remember = TRUE, $redirectTo = NULL){
      ...
      if (filter_var($username, FILTER_VALIDATE_EMAIL)) $email = true;
		...
		/* CONSULTA */
      $data = db_exec('fetch_assoc', db_exec(array(__FILE__, __LINE__), 'query', "SELECT user_id, user_name, user_password, user_activo, user_baneado FROM u_miembros WHERE $select LIMIT 1"));
      //
	...
	*/
	function userActivate($tsUserID = 0, $tsKey = 0){
		...
		if(db_exec('num_rows', $query) == 0 || $tsKey != $tsKeyLocal) return false;
		   ...
		   if(db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE u_miembros SET user_activo = 1 WHERE user_id = \''.$tsUserID.'\'')) return $tsData;
			else return false;
		}
	}
    /*
