...
class tsMod {
    ...
    */
    function getDenuncias($type = 'posts')
    {
        ...
        {
            case 'posts':
                $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT r.*, SUM(d_total) AS total, p.post_id, p.post_title, p.post_status, c.c_nombre, c.c_seo FROM w_denuncias AS r LEFT JOIN p_posts AS p ON r.obj_id = p.post_id LEFT JOIN p_categorias AS c ON p.post_category = c.cid WHERE d_type = 1 AND p.post_status < 2 GROUP BY r.obj_id ORDER BY total DESC, r.d_date DESC');
                $data = result_array($query);
    ...
    */
    function getDenuncia($type = 'posts')
    {
        ...
        switch ($type)
        {
            case 'posts':
                $d_type = 1;
                $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_id, p.post_title, p.post_status, c.c_nombre, c.c_seo, u.user_name FROM p_posts AS p LEFT JOIN p_categorias AS c ON p.post_category = c.cid LEFT JOIN u_miembros AS u ON p.post_user = u.user_id WHERE p.post_id = ' .
                    $obj . ' LIMIT 1');
                break;
            case 'fotos':
                $d_type = 4;
    ...
    function getContenido()
    {
        ...
        //
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_id, p.post_user, p.post_title, p.post_date, p.post_ip, u.user_name, c.c_nombre, c.c_seo FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE ' .
        ...
        $data['c_p_total'] = safe_count($data['p_comentarios']);
        //
        ...
        $data['c_f_total'] = safe_count($data['f_comentarios']);
        //
        ...
        if ($type == 'post')
        {
            switch ($action)
            {
                case 'borrar':
                    ...
                    // EDITAR
                case 'editar':
                    $aviso = 'Hola <b class="text-capitalize">' . $tsUser->getUserName($data['autor']) . "</b>\n Te informo que tu post <b>".$data['title']."</b> ha sido editado por <a href=\"#\" uid=\"".$tsUser->uid . "\">". $tsUser->nick ."</a>\n Causa: <b>" . $data['razon'] .
                        "</b>\n Te recomendamos leer el <a href=\"" . $tsCore->settings['url'] .
                        "/pages/protocolo/\">protocolo</a> para evitar futuras sanciones.\n Muchas gracias por entender!";
