...
class tsPosts {
         ...
         if ($tsUser->info['user_lastpost'] < (time() - $antiflood)) {
            ...
            if (db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `p_posts` (post_user, post_category, post_title, post_portada, post_body, post_date, post_tags, post_ip, post_private, post_block_comments, post_sponsored, post_sticky, post_smileys, post_visitantes, post_status) VALUES (\'' . $tsUser->uid . '\', \'' . (int) $postData['category'] . '\', \'' . $postData['title'] . '\', \'' . $postData['portada'] . '\', \'' . $postData['body'] . '\', \'' . $postData['date'] . '\', \'' . $postData['tags'] . '\', \'' . $_SERVER['REMOTE_ADDR'] . '\', \'' . (int) $postData['private'] . '\', \'' . (int) $postData['block_comments'] . '\', \'' . (int) $postData['sponsored'] . '\', \'' . (int) $postData['sticky'] . '\', \'' . (int) $postData['smileys'] . '\', \'' . (int) $postData['visitantes'] . '\', ' . (!$tsUser->is_admod && ($tsCore->settings['c_desapprove_post'] == 1 || $tsUser->permisos['gorpap'] == true) ? '\'3\'' : '\'0\'') . ')')) {
               ...
               if (!$tsUser->is_admod && ($tsCore->settings['c_desapprove_post'] == 1 || $tsUser->permisos['gorpap'] == true)) {
                  db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `w_historial` (`pofid`, `action`, `type`, `mod`, `reason`, `date`, `mod_ip`) VALUES (\'' . (int) $postID . '\', \'3\', \'1\', \'' . $tsUser->uid . '\', \'Revisi&oacute;n al publicar\', \'' . time() . '\', \'' . $_SERVER['REMOTE_ADDR'] . '\')');
               }
   ...
   */
   ...
   function getPost()
   {
      ...
      // CALIFICACIONES
      $query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT * FROM p_votos WHERE tid = '{$postData['post_id']}'");
   ...
   function deletePost()
   {
      ...
      if (!empty($data['post_id']) || !empty($tsUser->is_admod)) {
         ...
         if (db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_posts WHERE post_id = \'' . (int) $post_id . '\'')) {
            ...
            if (db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE p_posts SET post_status = \'2\' WHERE post_id = \'' . (int) $post_id . '\'')) {
   ...
   */
   function votarPost() {
      ...
      if ($tsUser->is_admod || $tsUser->permisos['godp']) {
         ...
         if ($tsUser->is_admod != 1) {
            if (db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_id FROM u_miembros WHERE user_last_ip =  \'' . $_SERVER['REMOTE_ADDR'] . '\' AND user_id != \'' . $tsUser->uid . '\'')) || db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT session_id FROM u_sessions WHERE session_ip =  \'' . $tsCore->setSecure($_SERVER['REMOTE_ADDR']) . '\' AND session_user_id != \'' . $tsUser->uid . '\''))) {
   ...
   function getFavoritos()
   {
      global $tsCore, $tsUser;
      //
      $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT f.fav_id, f.fav_date, p.post_id, p.post_title, p.post_date, p.post_puntos, COUNT(p_c.c_post_id) as post_comments,  c.c_nombre, c.c_seo FROM p_favoritos AS f LEFT JOIN p_posts AS p ON p.post_id = f.fav_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN p_comentarios AS p_c ON p.post_id = p_c.c_post_id && p_c.c_status = \'0\' WHERE f.fav_user = \'' . $tsUser->uid . '\' AND p.post_status = \'0\' GROUP BY c_post_id');
      $data  = result_array($query);
   ...
   function TituloAnterior() {
      ...
      if (db_exec('num_rows', $query) == 0) $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT post_id, post_title FROM p_posts WHERE post_id < \'' . (int) $post_id . '\' ORDER BY post_id DESC LIMIT 1');
      $data = db_exec('fetch_assoc', $query);
   ...
   function TituloSiguente() {
      ...
      if ($post_id <= 0) return false;
      $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT post_id, post_title FROM p_posts WHERE post_id = \'' . (int) $post_id . '\' LIMIT 1');
      if (db_exec('num_rows', $query) == 0) $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT post_id, post_title FROM p_posts WHERE post_id > \'' . (int) $post_id . '\' ORDER BY post_id ASC LIMIT 1');
      $data = db_exec('fetch_assoc', $query);
      return (!empty($data) ? $data : false);
   }
   # SEO FOR POSTS
