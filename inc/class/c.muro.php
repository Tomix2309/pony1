    ...
    public function ajaxCheck($return = false, $urlin = null){
        ...
        switch($type){
            ...
            case 'enlace':
                ...
                $tsOG = new getOpenGraph();
                $result = $tsOG->getUrlData($url, true);
                ...
                if($return == false) return '1: <div class="uiLink w-100"><strong class="d-block h4"><a href="'.$url.'" target="_blank" class="text-dark">'.$tsCore->setSecure($title[0], true).'</a></strong><span class="desc">'.$tsCore->setSecure($description, true).'</span></div>';
                else return array('title' => $tsCore->setSecure($title[0], true), 'url' => $tsCore->setSecure($url, true));
            break;
            ...
            case 'video':
                ...
                if(empty($data['title'])) return '0: La URL contiene un ID de video incorrecto o el video ha sido eliminado.';
                    ...
                    if($return == false)
                    return '1: <div class="vContent"><img src="https://img.youtube.com/vi/'.$video_id.'/0.jpg" class="w-100" alt="'.$data['title'].'"/><div class="vDesc"><a href="https://www.youtube.com/watch?v='.$video_id.'" target="_blank" class="font-weight-bolder">'.$data['title'].'</a><div style="margin-top:5px">'.$description.'</div></div></div>';
