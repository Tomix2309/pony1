...
<head>
<title>{$tsTitle}</title>
{metadatos facebook=true twitter=true}
{googlefonts fonts="Roboto"}
{jsdelivr type="css" files=['bootstrap','waitme','sweetalert2']}
{jsdelivr type="js" files='jquery'}
{hook css=['SyntaxisLite.min.css'] position="start"}
...
<body name="superior" id="body" class="py-3">
	...
	<br>
{if $tsConfig.c_reg_active === '1'}
{jsdelivr type="js" files=['bootstrap','waitme','sweetalert2']}
...
<script>
	$(() => {
	   ...
	   .then(() => loadScript('{$tsConfig.url}/public/assets/js/registro.js'))
