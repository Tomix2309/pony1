...
const portada = {
   cambiar: () => {
      ...
      $.post(global_data.url + '/perfil-cambiar.php', { pid }, response => {
         ...
         mydialog.buttons(true, true, 'Aplicar cambios', 'portada.actualizar()',true, true, true, 'No, quizas más tarde!', 'close', true, false);
         mydialog.center();
      })
   },
   actualizar: () => {
      const data = $('form[name=change_background_user]').serialize();
...
$(document).ready(() => $.getScript(global_data.tema + "js/cuenta.js"))
