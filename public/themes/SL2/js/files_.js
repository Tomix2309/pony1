		...
		$.ajax({		
			...
			success: function(h){
				switch(h.charAt(0)){
					...
					break;
					case '1':
						if($('#del_file').val() == 'true') location.href = global_data.url + '/files/';
						else {
		...
		success: function(h){
			if(h.charAt(0) == '0'){
				...
				$('#add_new_com').html(h).slideDown('slow', function () {
					...
					$('#alertcom_file').html('<div class="alert alert-success text-center">Tu comentario fue agregado correctamente.</div>');
				});
				// SUMAMOS
...
function print_editor() {
   ...
   if($('#file_newcom').length && !$('.wysibb-texarea').length){
      var wbbOpt = { buttons: "smilebox,|,bold,italic,underline" }
      $('#file_newcom').removeAttr('onblur onfocus class style title').css('height', '80').html('').wysibb(wbbOpt);
   }
}
print_editor();
...
function new_folder(action) {
	if (empty(action)) {
		...
		mydialog.body('<input type="text" class="form-input" placeholder="Nombre de la carpeta..." id="folder_name" size="55">');
				...
				success: function(h){
					switch(h.charAt(0)){
						...
						case '1': //OK
							...
							$('#ftotal').html(total + 1);
							$('#F_newfolder').html(h.substring(3));
							$('#alert, #loading').fadeOut(350);
							mydialog.procesando_fin();
							mydialog.close();
						break;
					}
		...
		$.ajax({
			...
			success: function(h){
				switch(h.folders){
					...
					case 1: //OK
						mydialog.procesando_fin();
						var html = '<div>Seleccionar carpeta a mover archivo</div>';
						html += '<select id="Folder_id" class="form-select" title="Seleccionar carpeta a mover">';
		...
		complete: function(response) {
			...
			switch(result.charAt(0)){
				...
				case '1':
					...
					mydialog.buttons(true, true, 'Cerrar', 'location.reload()', true, true);
					mydialog.center();
				break;
			}
		}
...
function last_files(page){
    ...
    $.get(global_data.url + '/files-last-files.php', 'page=' + page, function(h){
	   success: { $('#more_last_files').html(h).css('opacity', '1'); SL2.stop(); }
	});
    return false;
}
