       ...
       $.ajax({
    		...
    		success: function(h){
    			switch(h.charAt(0)){
                  ...
                  $('.form').css('display','block').html('<div class="alert alert-success text-center">Tu comentario fue agregado correctamente :)</div>');
    					// SUMAMOS
...
function last_files(page) {
   ...
   $.ajax({
      type: 'GET',
      url: global_data.url + '/fotos-publicadas.php?page=' + page,
      cache: false,
      success: function(h) {
         $('.nofio').css('opacity', 1);
         $('#com_gif').hide(100);
