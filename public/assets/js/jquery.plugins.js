...
class LiteYTEmbed extends HTMLElement {
   connectedCallback() {
      ...
      logosite.alt = global_data.titulo;
...
var mydialog = {
   ...
   buttons_action: (remBtn, dataObject) => { 
      ...
      if(dataObject.fail) {
         ...
         if(dataObject.fail.action === 'close' || !dataObject.fail.action) dataObject.fail.action = 'mydialog.close()';
         let classdisabled = dataObject.fail.active ? '' : ' disabled';
         is_html += `<input type="button" class="btn btn-danger mBtn btnCancel${classdisabled}" style="display:inline-block!important;" onclick="${dataObject.fail.action}" value="${dataObject.fail.text}"${classdisabled} />`;
      }
      // Si existe "OK"
      if(dataObject.ok) {
         // Si tiene accion definido
   ...
   procesando_inicio: function(value, title){
      ...
      $('#mydialog #procesando #mensaje').html('<span>Espere, cargando...</span>');
